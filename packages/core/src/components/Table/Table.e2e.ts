...
let SCREENSHOT_PREFIX = `${CORE_CUSTOM_SHOTS_PATH}widgets-bots-table--`;
...
const gotoPageNoControlOrders = (page: Page) => {
  return page.goto(
    'iframe.html?args=&id=components-table--no-control-orders&viewMode=story',
  );
};

SCREENSHOT_PREFIX = `${CORE_CUSTOM_SHOTS_PATH}widgets-bots-table--no-control-orders--`;
