...
const Default = (Component: React.ComponentType<any>) => {
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  function WrappedComponent(props: any) {
    const theme = useTheme();
    const { user } = useAuth();
    const { data: goals, query: loadGoals } = useApi(API.goals.get, []);
    const { data: workouts, query: loadWorkouts } = useApi(
      API.workouts.get,
      [],
    );
  ...
  return setHocDisplayName('Default', Component, WrappedComponent);
};
