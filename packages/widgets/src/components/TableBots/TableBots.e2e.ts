...
let SCREENSHOT_PREFIX = `${WIDGETS_CUSTOM_SHOTS_PATH}widgets-table-bots--`;

const gotoPage = (page: Page) => {
  return page.goto(
    'http://localhost:6007/iframe.html?args=&id=widgets-tablebots--table-bots&viewMode=story',
  );
};

test.describe('Widgets: TableBots', () => {
...
const gotoPageNoControlOrders = (page: Page) => {
  return page.goto(
    'http://localhost:6007/iframe.html?args=&id=widgets-tablebots--no-control-orders&viewMode=story',
  );
};

SCREENSHOT_PREFIX = `${WIDGETS_CUSTOM_SHOTS_PATH}widgets-table-bots--no-control-orders--`;

test.describe('Widgets: TableBots (NoControlOrders)', () => {
  test('Renders correctly', async ({ page }) => {
    await page.setViewportSize(SCREEN_SIZES[1920]);
