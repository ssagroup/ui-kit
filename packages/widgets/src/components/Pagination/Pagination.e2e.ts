...
const TEST_CASES = [
  {
    ...
    screenshotPrefix: 'widgets-pagination--selected-hover',
  },
  {
    ...
    screenshotPrefix: 'widgets-pagination--disabled-hover',
  },
];
...
for (const { url, testTitle, screenshotPrefix } of TEST_CASES) {
  test(testTitle, async ({ page }) => {
    ...
    await page.screenshot({
      path: `${WIDGETS_CUSTOM_SHOTS_PATH}${screenshotPrefix}__[w1920px].png`,
    });
